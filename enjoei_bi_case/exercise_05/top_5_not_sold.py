#!/usr/bin/env python3
"""top_5_not_sold.py: Top 5 categorias, subcategorias e departamentos não vendidos Rio de Janeiro"""
...
with open("database/products.csv", newline='') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        # Se o usuário que criou o anúncio, for do Rio de Janeiro e não vendeu o produto,
        # guardar informações de categoria, subcategoria e departamento
        if(row.get('user_id') in users_id):
            if(row.get('product_subcategory') != '' and row.get('state') != 'sold'):
...
# Mostrar no terminal o top 5 de não vendidos em categorias, subcategorias e departamentos no Rio de Janeiro
print('== Top 5 de produtos não vendidos ==')
